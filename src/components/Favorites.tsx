...
export const Favorites = () => {
  ...
  const filteredFavorites = favoriteCategory
    ? favorites.filter(
        (item) => favoriteCategory === item.category.toLowerCase()
      )
    : favorites;
              ...
              <ul className="flex gap-2 items-center flex-wrap">
                {favoriteCategory && (
                  <li
                    ...
                    className="text-black bg-secondary tracking-05 rounded-2xl  border border-violet px-3 py-1  hover:border-darkGreen hover:bg-green hover:text-black transition-all duration-300 cursor-pointer"
                  >
                    All
                  </li>
                )}
                {favorites?.map((item, index) => (
                  <li
                    ...
                    className={`text-black tracking-05 rounded-2xl border px-3 py-1 transition-all duration-300 cursor-pointer ${
                      favoriteCategory &&
              ...
              {filteredFavorites.map(({ id, meal, mealImg }) => (
                ...
                >
                  <div className="p-3 bg-pink">
                    ...
                    <LinkOrButton
                      type="button"
                      className="bg-red border-2 border-red rounded flex items-center justify-center gap-1 text-clearWhite cursor-pointer p-1  w-full hover:shadow-favorite hover:scale-103 transition-all duration-200"
